...
isLoading
{
    ...
    */
    var loadingConditions = new List<Tuple<uint, string>>{
        Tuple.Create(3221496410u, "dd_day_of_the_dead_050"), // Cutscene after leaving the temple tomb
        Tuple.Create(894107814u, "cm_croft_manor"), // At the top of croft manor
        Tuple.Create(969044434u, "lj_lost_in_the_jungle_v2_02"), // After standing up from camp (Broken the first time, works after a reload)
        Tuple.Create(3528814387u, "lj_lost_in_the_jungle_v2_connector_out"), // Crawling under the truck
        Tuple.Create(1233757502u, "pa_hub_paititi"), // Another broken cutscene
        Tuple.Create(1233757502u, "sp_sacrificial_pit"), // (^) persists throughout this whole segment
        Tuple.Create(1233757502u, "dm_devils_maw"), // (^) persists throughout this whole segment
        Tuple.Create(1692137511u, "sm_spanish_mission_village") // another broken cutscene
    };

    var isBad = false;
    foreach(var condition in loadingConditions)
    {
        var CutsceneValue = condition.Item1;
        var AreaValue = condition.Item2;
        if(current.Cutscene == CutsceneValue && current.Area == AreaValue)
        {
            isBad = true;
            break;
        }
    }
    if(current.Cutscene != 0 && !isBad)
    {
        return true;
    }
